...
export default function TransferPage() {
  ...
  if (step === "confirm" && formData) {
    return (
      <div className="space-y-6 max-w-2xl">
        <div>
          <h1 className="text-[22px] font-bold text-slate-900 tracking-tight">Confirm Transfer</h1>
          ...
          <div className="p-6 space-y-6">
            <div className="flex items-center justify-center py-6">
              <div className="flex items-center gap-5">
                ...
                <div className="flex flex-col items-center gap-1">
                  ...
                  <span className="text-[15px] font-bold text-blue-600">{formatCurrency(Number(formData.amount))}</span>
                </div>
                ...
                <div className="flex flex-col items-center gap-2">
                  ...
                  <span className="text-[11px] font-medium text-slate-400">User #{formData.receiverId}</span>
                </div>
              </div>
            </div>
            ...
            <div className="p-4 rounded-xl bg-slate-50 ring-1 ring-slate-100 space-y-3">
              <div className="flex justify-between text-[13px]">
                ...
                <span className="font-semibold text-slate-900">#{formData.receiverId}</span>
              </div>
              <div className="flex justify-between text-[13px]">
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div>
        <h1 className="text-[22px] font-bold text-slate-900 tracking-tight">Send Money</h1>
        <p className="text-[13px] text-slate-500 mt-0.5">Transfer money to another NeoBank user instantly</p>
      </div>

      <div className="bg-white rounded-xl border border-slate-200/80 overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100">
          <h2 className="text-[13px] font-semibold text-slate-900">Transfer Details</h2>
          <p className="text-[11px] text-slate-400 mt-0.5">Enter the recipient&apos;s user ID and amount to send</p>
        </div>

        <div className="p-5">
          <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-5">
            <div className="space-y-2">
              <Label htmlFor="receiverId" className="text-[13px] font-medium text-slate-700">Receiver User ID</Label>
              <Input
                id="receiverId"
                type="text"
                inputMode="numeric"
